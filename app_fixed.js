  ...
  const I18N = {
    en:{
      sub:"Mogux Retail Performance Suite", tenant:"Tenant", lang:"Language", reset:"Reset", login:"Login", logout:"Logout",
      demoMode:"Preview", welcome:"Welcome to Mogux",
      welcomeDesc:"Explore the full experience. In this preview, data is stored locally in your browser for privacy.",
      platformTitle:"Platform Admin",
      platformDesc:"Manage companies, sub-domains, branding, users, and standards from one secure control center.",
      tenants:"Tenants",
      ...
      quickLogin:"Quick login", quickHint:"Pick a role, then press “Enter demo”.", role:"Role", role_superadmin:"Super Admin", role_admin:"Admin", role_manager:"Branch manager", role_area:"City manager", role_sales:"Sales manager", role_employee:"Employee", role_shopper:"Mystery shopper", user:"User", enter:"Enter demo",
      ...
      eval_operational:"Operational evaluation",
      eval_probation:"Probation period evaluation",
      probationChecklist:"Probation checklist",
      kpi_probation:"Probation",
      copy:"Copy",
      ...
      overviewDesc:"Company → City → Branch → Employee",
      purposeTitle:"Designed to help you improve performance",
      purposeBody:"Mogux helps you develop operations, elevate customer service, and improve sales discipline — with audits, mystery shopping, and probation evaluations in one place.",
      worldClass:"World-class UX inspired by leading enterprise systems — simpler, faster, and mobile-first.",
      autoScore:"Score is calculated automatically based on weights.",
      colType:"Type",
      colDate:"Date",
      trendOps:"Operational trend",
      trendOpsDesc:"Average operational score over the last 14 days.",
      trendVisits:"Mystery Shopper trend",
      trendVisitsDesc:"Average shopper score over the last 14 days.",

    },
    ar:{
      sub:"Mogux | منصة أداء التجزئة", tenant:"الشركة", lang:"اللغة", reset:"إعادة ضبط", login:"دخول", logout:"تسجيل خروج",
      demoMode:"معاينة", welcome:"مرحبًا بك في Mogux",
      welcomeDesc:"استكشف التجربة كاملة. في وضع المعاينة يتم حفظ البيانات محليًا في متصفحك للحفاظ على الخصوصية.",
      quickLogin:"دخول سريع", quickHint:"اختر الدور ثم اضغط “دخول العرض”.", role:"الدور", role_superadmin:"سوبر أدمن", role_admin:"أدمن", role_manager:"مدير فرع", role_area:"مدير مدينة", role_sales:"مدير المبيعات", role_employee:"موظف", role_shopper:"عميل خفي", user:"المستخدم", enter:"دخول العرض",
      ...
      eval_operational:"تقييم التشغيل",
      eval_probation:"تقييم الانتظام في العمل",
      probationChecklist:"قائمة تقييم الانتظام",
      kpi_probation:"الانتظام",
      copy:"نسخ",
      ...
      nav_platform:"المنصة",
      overviewDesc:"الشركة ← المدينة ← الفرع ← الموظف",
      purposeTitle:"مصمم لمساعدتكم على تحسين الأداء",
      purposeBody:"Mogux موجود لضمان مساعدتكم على تطوير العمل، رفع جودة خدمة العملاء، وتحسين الأداء — عبر تقييمات تشغيلية، عميل خفي، وتقييم فترة التجربة في مكان واحد.",
      worldClass:"تجربة استخدام عالمية مستوحاة من أنظمة المؤسسات الكبرى — لكن أبسط وأسرع ومهيأة للموبايل.",
      autoScore:"يتم احتساب الدرجة تلقائيًا بناءً على الأوزان.",
      colType:"النوع",
      colDate:"التاريخ",
      trendOps:"اتجاه التشغيل",
      trendOpsDesc:"متوسط درجة التشغيل خلال آخر 14 يومًا.",
      trendVisits:"اتجاه العميل الخفي",
      trendVisitsDesc:"متوسط نتيجة العميل الخفي خلال آخر 14 يومًا."
  ...
  platformUsers:[{id:'sa_1', name:'Platform Admin', email:'admin@mogux.ca', role:'superadmin'}],
    ...
    probation:{
      ...
      colDate:"التاريخ",
      trendOps:"اتجاه التشغيل",
      trendOpsDesc:"متوسط درجة التشغيل خلال آخر 14 يوم.",
      trendVisits:"اتجاه العميل الخفي",
      trendVisitsDesc:"متوسط نتيجة العميل الخفي خلال آخر 14 يوم.",
...
function ensureTenantFields(){
  ...
  function ensurePlatformUsers(){
    if(!Array.isArray(state.platformUsers) || state.platformUsers.length===0){
      state.platformUsers = [{id:'sa_1', name:'Platform Admin', email:'admin@mogux.ca', role:'superadmin'}];
    }
  }
  ensurePlatformUsers();
  ...
  function currentUser(){
    if(state.session.role==='superadmin') return (state.platformUsers||[]).find(u=>u.id===state.session.userId) || null;
    return currentTenant().users.find(u=>u.id===state.session.userId) || null;
  }
...
function scopeLabel(){
  ...
  const scope=sessionScope();
  if(['admin','sales','superadmin'].includes(state.session.role)) return t('allAccess');
...
function showAuth(){ $('#auth').style.display='block'; $('#shell').style.display='none'; renderAuthSelectors(); }
  ...
  const ROUTES = {
    overview:{roles:['admin','sales','area','manager','employee','superadmin'], label:'nav_overview', render:renderOverview},
    reports:{roles:['admin','sales','area','manager','superadmin'], label:'nav_reports', render:renderReports},
    eval:{roles:['admin','sales','area','manager','superadmin'], label:'nav_eval', render:renderEvaluation},
    my:{roles:['employee'], label:'nav_my', render:renderMy},
    shop:{roles:['shopper'], label:'nav_shop', render:renderShopper},
    admin:{roles:['admin','superadmin'], label:'nav_admin', render:renderAdmin},
  ...
  function render(){
    ...
    const pb = document.getElementById('platformBtn');
    if(pb){
      pb.style.display = (state.session.role==='superadmin') ? 'inline-flex' : 'none';
      pb.onclick = ()=>{ state.session.route='platform'; saveState(); render(); };
    }
    ...
    const roleLabelMap = {
      superadmin: t('role_superadmin'), admin: t('role_admin'), sales: t('role_sales'),
      area: t('role_area'), manager: t('role_manager'), employee: t('role_employee'), shopper: t('role_shopper')
    };
    $('#whoRole').textContent = roleLabelMap[state.session.role] || state.session.role;

    if(!state.session.route || !ROUTES[state.session.route] || !ROUTES[state.session.route].roles.includes(state.session.role)){
      state.session.route = (state.session.role==='superadmin') ? 'platform' : (navItems(state.session.role)[0]?.key || 'overview');
      saveState();
    }
  ...
  function platformOpenTenant(tenantId){
    // In production: open https://<subdomain>.mogux.ca
    // In demo: keep superadmin logged in and switch tenant context.
    if(!Array.isArray(state.platformUsers) || state.platformUsers.length===0){
      state.platformUsers = [{id:'sa_1', name:'Platform Admin', email:'admin@mogux.ca', role:'superadmin'}];
    }
    state.session.tenantId = tenantId;
    state.session.userId = (state.platformUsers && state.platformUsers[0] ? state.platformUsers[0].id : 'sa_1');
    state.session.role = 'superadmin';
    state.session.route = 'overview';
    state.session.hostTenantLocked = true;
    saveState();
    syncTenantSelect();
    render();
    ...
    root.innerHTML = `
      <div class="row">
        <div>
          ...
          <div class="muted" style="margin-top:6px">${t('overviewDesc')}</div><div class="row" style="margin-top:10px; justify-content:flex-start; gap:10px; flex-wrap:wrap"><span class="pill">🔒 ${t('scope')}: ${escapeHtml(scopeLabel())}</span></div>
        </div>
        <div class="row" style="gap:10px; flex-wrap:wrap; justify-content:flex-end">
          ${state.session.role!=='employee' ? `<button class="btn" onclick="setRoute('reports')">📊 ${t('nav_reports')}</button>` : ''}
          ${(['admin','superadmin'].includes(state.session.role)) ? `<button class="btn primary" onclick="setRoute('admin')">⚙️ ${t('nav_admin')}</button>` : ''}
        </div>
      </div>
      <div class="divider"></div>
      <div class="card" style="margin-top:14px">
        <div class="row" style="align-items:flex-start">
          <div style="min-width:0">
            <h3 style="margin:0">✨ ${t('purposeTitle')}</h3>
            <div class="muted" style="margin-top:8px; font-size:13px; line-height:1.75">${t('purposeBody')}</div>
            <div class="muted" style="margin-top:8px; font-size:13px; line-height:1.75">${t('worldClass')}</div>
          </div>
        </div>
      </div>
  ...
  ensurePlatformUsers();
    ...
    function sparkSvg(values){
      ...
      const v = (values && values.length) ? values : [62,64,61,66,68,65,69,72,70,74,76,73];
      const min=Math.min(...v), max=Math.max(...v);
      const sx=(i)=> pad + (i*(w-2*pad))/Math.max(1,(v.length-1));
      const sy=(x)=> pad + (h-2*pad) * (1-((x-min)/(max-min||1)));
      const pts=v.map((x,i)=>`${sx(i)},${sy(x)}`).join(' ');
      const last=v[v.length-1];
      return `<svg viewBox="0 0 ${w} ${h}" class="spark" aria-hidden="true">
        <defs>
          <linearGradient id="g" x1="0" x2="1">
            <stop offset="0" stop-color="rgba(185,150,74,.18)"/>
            <stop offset="1" stop-color="rgba(11,18,32,.10)"/>
          </linearGradient>
        </defs>
        <polyline points="${pts}" fill="none" stroke="var(--blueblack)" stroke-opacity=".78" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
        <polygon points="${pts} ${w-pad},${h-pad} ${pad},${h-pad}" fill="url(#g)" />
        <circle cx="${sx(v.length-1)}" cy="${sy(last)}" r="4" fill="var(--gold)" />
      </svg>`;
    }
    function lastNDaysSeries(ev, keyFn, days=14){
      const now = new Date();
      const buckets=[];
      ...
      (ev||[]).forEach(e=>{
        ...
        if(!b) return;
        const v = keyFn(e);
        if(v!=null && v!=='' && !isNaN(Number(v))) b.vals.push(Number(v));
      });
      const series = buckets.map(b=> b.vals.length ? Math.round(b.vals.reduce((a,x)=>a+x,0)/b.vals.length) : null);
      return series.map((x,i)=> x!=null ? x : (i>0 && series[i-1]!=null ? series[i-1] : 70));
    }
    ...
    root.innerHTML = `
      ...
      <div class="grid kpis" style="margin-top:14px">
        ...
        ${kpiCard(t('kpi_risk'), riskCount, riskCount===0?'good':'bad')}
            ...
            <div class="grid c2" style="margin-top:14px">
        <div class="card">
          <div class="row"><div style="min-width:0"><h3 style="margin:0">${t('trendOps')}</h3><div class="muted" style="margin-top:6px">${t('trendOpsDesc')}</div></div><span class="pill">14d</span></div>
          <div class="divider"></div>
          ${sparkSvg(lastNDaysSeries(ev.filter(x=>x.type==='manager'), e=>Number(e.score||0), 14))}
        </div>
        <div class="card">
          <div class="row"><div style="min-width:0"><h3 style="margin:0">${t('trendVisits')}</h3><div class="muted" style="margin-top:6px">${t('trendVisitsDesc')}</div></div><span class="pill">14d</span></div>
          <div class="divider"></div>
          ${sparkSvg(lastNDaysSeries(ev.filter(x=>x.type==='shopper'), e=>Number(e.score||0), 14))}
        </div>
      </div>
...
<div class="card" style="margin-top:14px">
      ...
      <div class="card" style="margin-top:14px">
        ...
        ${ev.length ? `
          <table>
            <thead><tr><th>${t('colType')}</th><th>${t('colDate')}</th><th>${t('filter_city')}</th><th>${t('filter_branch')}</th><th>${t('filter_employee')}</th><th>${t('score')}</th></tr></thead>
            <tbody>
              ${ev.slice(0,12).map(e=>{
...
function scoreFromChecklist(prefix, role){
  ...
  function renderChecklist(prefix, role){
    ...
    return `
      ...
      <div class="muted">${t('autoScore')}</div>
      <div class="divider"></div>
      <div class="form">
      ...
      <div class="grid c2">
        <div class="card">
          <div class="form">
            ...
            <div class="field"><label>${t('evalType')}</label><select id="evType"><option value="eval_operational">${t('eval_operational')}</option><option value="eval_probation">${t('eval_probation')}</option></select></div>
<div class="field"><label>${t('filter_employee')}</label><select id="evEmp">${employees.map(u=>`<option value="${u.id}">${u.name}</option>`).join('')}</select></div>
    ...
    const syncEvalType = ()=>{
      ...
      const role = (tp==='eval_probation') ? 'probation' : 'manager';
      const sub = document.getElementById('evSub');
      if(sub) sub.textContent = (tp==='eval_probation') ? t('eval_probation') : t('eval_operational');
      $('#evChecklist').innerHTML = renderChecklist('ev_', role);
    };
    ...
    $('#evSubmit').onclick = async ()=>{
      ...
      const tp = $('#evType').value;
      const role = (tp==='eval_probation') ? 'probation' : 'manager';
      const {score, breakdown, answers} = scoreFromChecklist('ev_', role);
      const photos=[];
      if(photoFile) photos.push(await fileToDataUrl(photoFile));

      ten.evaluations.push({id:uid('ev'), type: tp==='eval_probation' ? 'probation' : 'manager', date:Date.now(), cityId, branchId, employeeId, score, breakdown, answers, notes, photos});
