...
function scopeLabel(){
  ...
  function syncTenantSelect(){
    const sel=$('#tenantSelect'); sel.innerHTML='';
    const role = state.session.role;
    ...
    list.forEach(ten=>{
      const opt=document.createElement('option'); opt.value=ten.id; opt.textContent=`${ten.companyName} (${ten.subdomain})`; sel.appendChild(opt);
    });
    ...
    sel.onchange=()=>{
      if(!canSwitch) return;
      state.session.tenantId=sel.value; state.session.userId=null; state.session.role=null; state.session.route=null; 
      state.session.hostTenantLocked=false;
      saveState(); render();
    };
  });
    sel.value=state.session.tenantId;
    sel.onchange=()=>{ state.session.tenantId=sel.value; state.session.userId=null; state.session.role=null; state.session.route=null; saveState(); render(); };
  }
